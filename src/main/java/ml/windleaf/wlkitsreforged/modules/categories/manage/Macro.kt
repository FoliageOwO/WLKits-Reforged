package ml.windleaf.wlkitsreforged.modules.categories.manage
...
import ml.windleaf.wlkitsreforged.modules.categories.manage.macro.MacroException
import ml.windleaf.wlkitsreforged.modules.categories.manage.macro.MacroManager
...
@ModuleInfo(category = Category.MANAGE, description = "Run the macro", type = LoadType.ON_STARTUP)
            ...
            when (args[0]) {
                ...
                "authed" -> {
                    if (args.size != 2) Util.invalidArgs(sender)
                        ...
                        else Util.send(sender, Util.insert(Util.getModuleMsg(getName(), "authed"), "playerName" to n, "authed" to Util.parseBooleanColor(
                            manager.isAuthorized(player))))
                    }
                }
